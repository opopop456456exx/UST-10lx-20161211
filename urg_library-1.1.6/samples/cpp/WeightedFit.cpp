...
int HoughArc2(int X[] , int Y[] , int Cnt ,int r, ArcPara * Arc){
	...
	double deltaTheta = PI/180;//间隔1度
	double startAngle = 150.0*PI/180;
	double endAngle = PI*2 + PI/6;                //150度~390度，因为激光雷达扫描柱子只会扫描到前面这些角度，这是去掉后方的
	minA = maxA = X[0] - r;
	minB = maxB = X[0]; //theta = 0
	int halfWindowSize = 3;                    //半窗口大小？
	//计算a，b的最小和最大值
	for (int i = 0; i < Cnt;i++)
	{
		for (theta = startAngle; theta < endAngle;theta += deltaTheta)
		{
			a = (int)(X[i] - r*cos(theta) + 0.5);             //加0.5是为了向上取整？
			b = (int)(Y[i] - r*sin(theta) + 0.5);
			...
			else if (a - halfWindowSize< minA)                //X坐标的上下限
			{
	...
	//printf("%d,%d\n",aScale, bScale);
	//int *VoteBox = new int[aScale*bScale];l
	//VoteBox初始化为0
	for (int i = 0; i < aScale*bScale;i++)
	{
		VoteBox[i] = 0;
	...
	for (int i = 0; i < Cnt;i++)
	{
		...
		for (theta = startAngle; theta < endAngle;theta += deltaTheta)
		{
			...
			a = (int)(X[i] - r*cos(theta) + 0.5);                                  // 又算了一遍，可以优化？
			b = (int)(Y[i] - r*sin(theta) + 0.5);	
	...
	//释放内存
	//delete [] VoteBox;
	//printf("%d\n", (2 * halfWindowSize + 1)*(2 * halfWindowSize + 1) * 2);  值为98
